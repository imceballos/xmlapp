...
async def UPDATE_MESSAGE_ACCRUALS_READ(file: str):
    """
    Take the XML file UPDATE_MESSAGE_ACCRUALS.xml in string format, read it and return a dictionary with its field names as keys and its contents as values.
    """
    ...
    ChargeLine_ChargeLine_CostOSCurrency_Code = ChargeLine_CostOSCurrency.find(
        "Code"
    ).text
    result[
        "ChargeLine_ChargeLine_CostOSCurrency_Code"
    ] = ChargeLine_ChargeLine_CostOSCurrency_Code
...
async def UPDATE_MESSAGE_ACCRUALS_WRITE(data: dict, folder_path: str):
    """
    Take a dictionary and create an XML file with its information using the format of the XML file UPDATE_MESSAGE_ACCRUALS.xml.
    """
